...
func TestChatNewCompletionWithOptionalParams(t *testing.T) {
	...
	_, err := client.Chat.NewCompletion(context.TODO(), relaxaitest.ChatNewCompletionParams{
		Messages: []relaxaitest.ChatCompletionMessageParam{{
			...
			Role:    "role",
			Content: relaxaitest.String("content"),
		...
		Prediction: relaxaitest.ChatNewCompletionParamsPrediction{
			Content: "content",
			Type:    "type",
		},
		PresencePenalty: relaxaitest.Float(0),
		ReasoningEffort: relaxaitest.String("reasoning_effort"),
		ResponseFormat: relaxaitest.ChatNewCompletionParamsResponseFormat{
			JsonSchema: relaxaitest.ChatNewCompletionParamsResponseFormatJsonSchema{
				Name: "name",
		...
		Tools: []relaxaitest.ChatNewCompletionParamsTool{{
			Type: "type",
