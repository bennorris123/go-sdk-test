...
```sh
go get -u 'github.com/bennorris123/go-sdk-test@v0.1.0'
```
...
The relaxaitest library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `relaxaitest.String(string)`, `relaxaitest.Int(int64)`, etc.
...
```go
p := relaxaitest.ExampleParams{
	ID:   "id_xxx",                  // required property
	Name: relaxaitest.String("..."), // optional property

	Point: relaxaitest.Point{
		X: 0,                  // required field will serialize as 0
		Y: relaxaitest.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: relaxaitest.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[relaxaitest.FooParams](12)
```
...
client.Chat.NewCompletion(context.TODO(), ...,
	// Override the header
...
`*relaxaitest.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `relaxaitest.File(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ...
    Data: FooNewParamsData{
        FirstName: relaxaitest.String("John"),
    },
}
